...
class BaltiniScrapper(shopify):
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(BaltiniScrapper, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def GetProductUrls(self, homePageResponse):
        ...
        topCategoryNodes = homePageResponse.xpath(
            "//ul[contains(@class,'site-navigation')]/li[a[contains(text(),'WOMEN') or contains(text(),'MEN')]]")
        for topCategoryNode in topCategoryNodes:
            topCategoryTitle = topCategoryNode.xpath("./a/text()").get().strip()
            categoryNodes = topCategoryNode.xpath(
                ".//div[@class='h5 ' and contains(a/text(),'CLOTHING') or contains(a/text(),'NEW')]")
            for categoryNode in categoryNodes:
                ...
                subCategoryNodes = categoryNode.xpath(
                    "./following-sibling::div/a[contains(text(),'Dresses') or contains(text(),'Jumpsuits') or contains(text(),'Suits')]")
    ...
    def listing(self, categorylink, category):
        ...
        product_list = categoryPageResponse.xpath(
            "//a[contains(@class,'product__link')]/@href").extract()
    ...
    def GetProducts(self, response):
        ...
        if (re.search('NEW ARRIVALS', categoryAndName, re.IGNORECASE)) and not \
                re.search(r'\b((shirt(dress?)|jump(suit?)|dress|gown|suit|kaftan|caftan)(s|es)?)\b', categoryAndName,
                          re.IGNORECASE):
    ...
    def GetName(self, response):
        try:
            ...
            return shopify.GetName(self, response)

    def GetSelectedColor(self, response):
        try:
            return response.xpath("//p[contains(strong/text(),'Color')]/text()").get().strip()
        except:
            pass
