...
class Currentelliottscrapper(shopify):
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(Currentelliottscrapper, cls).from_crawler(crawler, *args, **kwargs)
        crawler.signals.connect(spider.spider_closed, signal=signals.spider_closed)
        return spider

    def GetProductUrls(self, homePageResponse):
        # ========== Category And Subcategory ==========#
        topCategoryNodes = homePageResponse.xpath("//nav/ul/li[a[contains(text(),'CLOTHING')]]")
        for topCategoryNode in topCategoryNodes:
            topCategoryTitle = topCategoryNode.xpath("./a/text()").get().strip()
            topCategorylink = topCategoryNode.xpath("./a/@href").get()
            categoryNodes = topCategoryNode.xpath(
                "./div/div/div[a[contains(text(),'BOTTOMS')]]")
            for categoryNodes in categoryNodes:
                ...
                subCategoryNodes = categoryNodes.xpath(
                    "./ul/li/a[contains(text(),'JUMPSUITS') or contains(text(),'DRESSES')]")
    ...
    def listing(self, categorylink, category):
        ...
        product_list = categoryLinkResponse.xpath("//div[@class='ProductItem__Wrapper']/a/@href").extract()
    ...
    def GetName(self, response):
        color = str(response.xpath("//p[@class='color_name']/text()").get()).replace('COLOR', '').replace('/',
                                                                                                          '').replace(
            'Color', '').strip()
