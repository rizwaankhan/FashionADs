...
class CupsheScrapper(shopify):
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(CupsheScrapper, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def GetProductUrls(self, homePageResponse):
        ...
        topCategoryNodes = homePageResponse.xpath(
            "//nav[contains(@class,'Header__MainNav')]/ul/li[a[not (contains(text(),'Bikini')) and not (contains(text(),'Seller'))]]")
        for topCategoryNode in topCategoryNodes:
            try:
                topCategoryTitle = topCategoryNode.xpath("./a/text()").get().strip()
            except:
                topCategoryTitle = topCategoryNode.xpath("./a/span/strong/text()").get().strip()
            topCategorylink = topCategoryNode.xpath("./a/@href").get()
            if not topCategorylink.startswith(store_url):
                topCategorylink = store_url.rstrip('/') + topCategorylink
            categoryNodes = topCategoryNode.xpath(
                "./div/div/div[a[contains(text(),'Swim') or contains(text(),'Clothing') or contains(text(),'One') or contains(text(),'Dress') or contains(text(),'Style') or contains(text(),'Sale') or contains(text(),'TOPS')]]")
            for categoryNode in categoryNodes:
                ...
                subCategoryNodes = categoryNode.xpath(
                    "./ul/li/a[contains(text(),'One') or contains(text(),'Dress') or contains(text(),'Jumpsuit') or contains(text(),'Bodysuits') or contains(text(),'Cover Ups') or contains(text(),'Matching')]")
