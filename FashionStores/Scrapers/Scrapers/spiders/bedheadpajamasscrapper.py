...
class BedheadPajamasScrapper(shopify):
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(BedheadPajamasScrapper, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def GetProductUrls(self, homePageResponse):
        ...
        topCategoryNodes = homePageResponse.xpath(
            "//nav[@class='theme_nv']/ul/div[@class=' dropdown']/li[a[contains(span/a/text(),'WOMEN') or contains(span/a/text(),'MEN') or contains(span/a/text(),'CHILDREN')]]")
        for topCategoryNode in topCategoryNodes:
            topCategoryTitle = topCategoryNode.xpath("./a/span/a/text()").get().strip()
            topCategorylink = topCategoryNode.xpath("./a/span/a/@href").get()
            if not topCategorylink.startswith(store_url):
                topCategorylink = store_url.rstrip('/') + topCategorylink
            categoryNodes = topCategoryNode.xpath(
                "./div//ul/li[a[contains(span/text(),'SHOP')]]")
    ...
    def listing(self, categorylink, category):
        ...
        product_list = categoryPageResponse.xpath(
            "//a[@class='product-title']/@href").extract()
